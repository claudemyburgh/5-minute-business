...
<section class="my-24 wrapper business_generator_section">
    <x-text.heading-2>
    ...
    <div class="shadow-lg flex items-center justify-center shadow-black/20 my-4 text-md font-mono min-h-[400px] isolate border border-primary-500/50 bg-gray-900 p-8 rounded-xl relative overflow-clip">
        ...
        <div id="highlight-container" class="flex flex-wrap pointer-events-none select-none gap-4 absolute inset-8 -z-10 ">
            @foreach(collect($business_names)->take(40) as $name)
                <span class="even:text-primary-500 odd:rotate-2 even:-rotate-1 highlight-item opacity-5">{{ $name }}</span>
            @endforeach
